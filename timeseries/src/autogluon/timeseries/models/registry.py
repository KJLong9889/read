...
class ModelRegistry(ABCMeta):
    ...
    def _get_model_record(cls, alias: str | type) -> ModelRecord:
        if isinstance(alias, type):
            name = alias.__name__.removesuffix("Model")
            if name not in cls.REGISTRY:
                # 允许传入的模型类动态注册，避免子进程或不同导入顺序导致 Unknown model
                try:
                    from autogluon.timeseries.models.abstract import AbstractTimeSeriesModel
                except ImportError:
                    pass
                else:
                    if issubclass(alias, AbstractTimeSeriesModel):
                        cls._add(
                            name,
                            ModelRecord(
                                model_class=alias,
                                ag_priority=getattr(alias, "ag_priority", 0),
                            ),
                        )
                    else:
                        raise ValueError(
                            f"Cannot register {alias.__name__}: not a subclass of AbstractTimeSeriesModel."
                        )
            alias = name
        else:
