...
class TimeSeriesTrainer(AbstractTrainer[TimeSeriesModelBase]):
    ...
    def tune_model_hyperparameters(
        ...
        # 只保留指标最优的一个超参 trial，并以基础模型名（如 AutoARIMA）注册，避免 AutoARIMA/T2、AutoARIMA/T3 导致库中命名不一致和权重文件过多
        valid_hpo = [
            (model_hpo_name, model_info)
            for model_hpo_name, model_info in hpo_models.items()
            if "val_score" in model_info
            and Path(os.path.join(self.path, model_info["path"])).exists()
        ]
        if not valid_hpo:
            return model_names_trained

        best_hpo_name, best_model_result = max(valid_hpo, key=lambda x: x[1]["val_score"])
        model_path = os.path.join(self.path, best_model_result["path"])
        model_hpo = self.load_model(best_hpo_name, path=model_path, model_type=type(model))
        ...
        model_hpo.name = model.name
        self._add_model(model_hpo)
        model_names_trained.append(model_hpo.name)

        logger.info(f"\tTrained 1 best model (by validation score) while tuning {model.name}; kept as '{model.name}'.")
        ...
        # 删除非最优 trial 的目录，减少磁盘上的权重文件
        for model_hpo_name, model_info in hpo_models.items():
            if model_hpo_name == best_hpo_name:
                continue
            trial_path = os.path.join(self.path, model_info["path"])
            if Path(trial_path).exists():
                try:
                    shutil.rmtree(trial_path)
                    logger.debug(f"\tRemoved non-best trial directory: {trial_path}")
                except OSError as e:
                    logger.warning(f"\tCould not remove trial directory {trial_path}: {e}")
    ...
    def _predict_model(
        ...
        This method assumes that model_pred_dict contains the predictions of all base models, if model is an ensemble.
        """
        # 始终用“图里的模型名”去解析依赖，避免像 PatchTST/acd85_00000 这种磁盘内部名
        # 与 model_graph 节点名不一致时，get_minimum_model_set 里的 BFS 抛 NetworkXError
        model_name = model if isinstance(model, str) else model.name

        # 先基于模型名从 model_graph 计算所需的 base model 集合
        model_inputs = self._get_inputs_to_model(model=model_name, data=data, model_pred_dict=model_pred_dict)

        # 然后再加载/使用真正的模型对象进行 predict
        model_obj = self.load_model(model_name) if isinstance(model, str) else model
        return model_obj.predict(model_inputs, known_covariates=known_covariates)
