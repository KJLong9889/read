...
def train_model(
    ...
    # 1. 保存上传文件到临时目录 temp/training/{task_id}/
    ...
    # ================= 核心修改 =================
    # 使用 parse_json 处理前端传来的 JSON 字符串，反序列化为 Python 对象
    params = {
        "target": target,
        ...
        # 默认值为 []，对接 parse_hpo_config 的列表结构解析
        "hyperparameters": parse_json(hyperparameters, []),
        
        # 将前端字符串如 '["weighted", "simple"]' 转化为列表
        "ensemble_types": parse_json(ensemble_types, []),
        
        # 将前端的 Ray Tune 配置转化为字典
        "hyperparameter_tune_kwargs": parse_json(hyperparameter_tune_kwargs, None)
    }
    # ============================================

    try:
        # 执行训练 (内部会自动调用 hpo_template 并生成榜单文件)
        model_folder = train_service.run_training_logic(task_id, backup_path, params)
        
        # 打包整个文件夹 (包含了模型文件 + evaluation_results 文件夹)
        zip_filename = f"{task_id}_model_pack.zip" 
