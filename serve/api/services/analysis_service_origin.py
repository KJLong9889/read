...
def worker_generate_features(task_id: str, file_path: str, max_timeshift: int, min_timeshift: int, tsfresh_custom_params: str):
    """后台任务：生成全量特征"""
    ...
    try:
        ...
        if tsfresh_custom_params:
            logger.info(f"检测到自定义参数配置，正在解析...")
            try:
                temp = json.loads(tsfresh_custom_params)
                parsed_params = {}
                for k, v in temp.items():
                    # 处理 ["'{lag}' for lag in range(1,4)"] 这种动态字符串
                    if isinstance(v, list) and len(v) == 1 and isinstance(v[0], str) and "for" in v[0]:
                        try:
                            # 传入 range 到 eval 环境
                            parsed_params[k] = eval(f"[{v[0]}]", {"range": range})
                            logger.info(f"  - 动态参数解析成功: {k}")
                        except Exception as parse_err:
                            logger.warning(f"  - 动态参数 {k} 解析失败，使用原始值: {parse_err}")
                            parsed_params[k] = v
                    else:
                        parsed_params[k] = v
                logger.info("自定义参数解析完成。")
            except Exception as e:
                logger.warning(f"自定义参数 JSON 解析失败: {e}，将使用默认 'comprehensive' 配置")
